...
description: "Migration plan for apps/qwk-in-lobe powering chat mode and settings."
---

# LobeHub Packages Migration Plan — Chat Engine + Settings Cutover

Step-by-step plan for making [`apps/qwk-in-lobe/`](https://github.com/OpenSourceAGI/qwksearch-research-agent/blob/main/apps/qwk-in-lobe/README.md) (the
...
- [`LOBEHUB_CORE_ENGINE_PLAN.md`](./lobehub-core-engine-plan) proposed this
  ...
  **This plan replaces it**, grounded in `apps/qwk-in-lobe/` — which is no longer a raw
...
Status: **plan — `apps/qwk-in-lobe/` builds and deploys; no traffic cutover yet.**

---

## 0. Where we actually are (baseline)

`apps/qwk-in-lobe/` already implements a large share of the old core-engine plan:

| Concern | State in `apps/qwk-in-lobe/` today |
|---|---|
...
| Extraction | `worker/qwksearch/extract.ts`: **`extract-webpage`** → Cloudflare Puppeteer scraper → Tavily → `@lobechat/web-crawler`, routed per URL kind — YouTube goes to `extract-youtube`, PDF/arXiv to `extract-pdf` |
...
`apps/qwk-in-lobe/README.md` before the Phase 4 public cutover.
...
│
├── Worker B: apps/qwk-in-lobe       (the engine)
...
(`packages/*`, `apps/*`) and the pnpm workspace in `apps/qwk-in-lobe/`. Do **not**
...
- **1.1 Stand up staging.** `cd apps/qwk-in-lobe && pnpm install && bun run build:worker
...
- **1.3 Extraction chain, first tier = article extractor.** ✅ Done, but not the
  way this plan first proposed. Tier 0 was going to be an HTTPS call to Worker
  A's `GET /api/doc/article`. Instead the engine imports `extract-webpage`
  directly (`worker/qwksearch/extractQwkSearch.ts`) and calls `extractContent`
  in-process. Three reasons: the engine already serves its own
  `/api/doc/article` against the *same* D1 `articleCache`, so the network hop
  bought no cache sharing that the binding did not already give; it would have
  made every extraction depend on Worker A being up, which runs against the
  point of the cutover; and 4.5 decommissions Worker A's agent routes anyway,
  so tier 0 would have had to be rewritten later. YouTube and PDF/arXiv route
  to `extract-youtube` / `extract-pdf` through the extractor's own branching
  rather than being rejected up front. See the
  [To-Do](./lobehub-migration-todo) §1.3 for what it cost (`linkedom` un-shimmed,
  +0.54 MB gzipped) and what was left open.
...
`apps/qwk-in-lobe/README.md` already names this the intended follow-up: Docs uses a
...
- ✅ Check: root `bun run build` + `bun run test` green with the trimmed graph;
  `apps/qwk-in-lobe` CI job builds SPA + worker and runs its vitest suites.
...
| Two UI systems bleed (antd 6 vs Tailwind 4) | Seam rule + the REASON editor's scoped CSS; no cross-imports between `apps/qwk-in-lobe/src` and `packages/research-agent-ui` |
| Worker size limit (10 MB gzipped, currently ~7.4) | CI size budget; lazy-load heavy SPA chunks; keep extractor code in Worker A |
| Postgres ops burden (169 tables, pgvector) | Managed (Neon/Hyperdrive) — already the tested path in `apps/qwk-in-lobe`; D1 stays for QwkSearch tables |
...
| Fork drift vs upstream LobeHub | Keep QwkSearch changes additive (`worker/`, `src/features/QwkSearch/`, new settings tabs); the "What changed vs. upstream" list in `apps/qwk-in-lobe/README.md` is the merge contract |
