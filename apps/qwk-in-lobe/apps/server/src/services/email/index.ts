...
import { hasCfBinding } from '@/libs/cloudflare/env';
  ...
  constructor(implType?: EmailImplType) {
    ...
    const envImplType =
      typeof window === 'undefined'
        ...
        : undefined;
    // On Cloudflare Workers there is no SMTP socket: default to the Email
    // Routing binding when it is bound and no provider was configured.
    const platformDefault = hasCfBinding('EMAIL')
      ? EmailImplType.Cloudflare
      : EmailImplType.Nodemailer;
    const resolvedImplType = implType ?? envImplType ?? platformDefault;
