---
title: "LobeHub as the Core Engine"
description: "Superseded integration plan for inverting the architecture around apps/qwk-in-lobe."
---

# LobeHub as the Core Engine — Integration Plan

> **Superseded** by [`LOBE_PACKAGES_MIGRATION_PLAN.md`](./lobe-packages-migration-plan),
> which re-grounds this strategy in the now-working `apps/qwk-in-lobe/` Cloudflare port
...
- Vendored source baseline: `packages-third-party/` (removed; see note above)
  — all 96 LobeHub workspace packages at `vtempest/lobehub@25a7c3fd` (v2.2.13), unmodified.
- Relationship to [`LOBEHUB_INTEGRATION_PLAN.md`](./lobehub-integration-plan) (#286):
...
| REASON writing editor | `reason-editor` (see [`REASON_EDITOR.md`](/docs/reason-editor)) | LobeHub's editor is `@lobehub/editor` (chat input); REASON is a full document organizer |
...
- **0.2** ✅ This plan + [`REASON_EDITOR.md`](/docs/reason-editor).
...
Details in [`REASON_EDITOR.md`](/docs/reason-editor); summary:
